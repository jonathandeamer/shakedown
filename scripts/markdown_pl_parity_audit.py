...
def format_report_path(path: Path) -> str:
    """Render paths in a stable, home-relative form for reports."""
    home = Path.home()
    try:
...
def unified_text_diff(expected_path: Path, expected: bytes, oracle: bytes) -> str:
    ...
    diff = "".join(
        difflib.unified_diff(
            ...
            fromfile=format_report_path(expected_path),
    ...
    return (
        "--- "
        f"{format_report_path(expected_path)}\n"
        "+++ local Markdown.pl\n"
        "@@ byte-diff @@\n"
...
def render_report(
    ...
    lines = [
        ...
        "",
        f"- **Fixtures:** `{format_report_path(fixtures_dir)}`",
        f"- **Oracle:** `{format_report_path(markdown_pl)}`",

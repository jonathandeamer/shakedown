...
## Autonomous loop fast-path (per iteration)

An MCO loop iteration (`./agent-loop`) should **not** read the full reading
order below. Read only:

1. [`superpowers/plans/plan-roadmap.md`](superpowers/plans/plan-roadmap.md) — the live roadmap; its table is the machine-parsed source of truth for the sole in-flight plan.
2. The in-flight plan it names (under `superpowers/plans/`) — your unit of work is one unchecked step plus its evidence gate.
3. `../.agent/blockers.md` — halt switch; any `- BLOCK:` line takes priority.
4. Only the specific truth docs the active step cites (e.g. `spl/reference.md` for SPL legality, `spl/literary-spec.md` for literary surfaces). Do not read the heavy docs speculatively — `spl/literary-spec.md` and `verification-plan.md` alone are ~20k tokens.

Loop mechanics live in [`2026-07-12-mco-loop-details.md`](2026-07-12-mco-loop-details.md).

## Reading Order for a New Agent (interactive planning sessions)

This full reading order is for an **interactive planning or architecture
session**, not for a per-iteration autonomous executor (use the fast-path
above). For an agent about to plan Shakedown implementation work, read the
canonical docs in this order:
...
| `ralph-loop.md` | Historical Huntley/Ralph loop methodology reference (superseded by the MCO loop), with lineage lessons. |
| `../agent-loop` | Active MCO-backed roadmap executor. |
...
- It does not treat loop prompts as design specs. The MCO loop's per-iteration prompt is generated by `scripts/mco_loop.py` from the roadmap's active-plan state; the legacy `run-loop` prompt is archived at `archive/prompt-shakedown-legacy.md`.

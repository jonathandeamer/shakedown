...
| 4S | Span Architecture Spike (`docs/superpowers/plans/2026-07-12-span-architecture-spike.md`) | Completability hardening §4 | Establish protected-region and buffered-scan shape before broad Act III growth: variable code spans, escapes, protected HTML/link/image regions, representative overlapping strong/emphasis | Reviewed span streams and oracle-backed probes pass; Act III model confirmed or halt-and-redesign triggered; Spike A/B snippets remain green. Amended 2026-07-13: Amendment A1 authorizes the two-character code-span machine and expanded literary pool (the twenty Step 5 halt records are resolved). Amendment A2 (2026-07-13) retires the original 23+6 Task 4 pool and replaces it with a shared-idiom design (one `LYRIC_FIELD_*` pipeline, capture-hold-then-requeue for label/alt, duplicate-on-reverse for autolink) plus a 41+10 pool, resolving the Task 4 Step 2 `BLOCK[plan]` (91-scene WIP vs 29-title budget); fix/implement executors may resume Task 4 Step 2 | in flight |

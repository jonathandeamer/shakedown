...
| 10 | Malformed Reference Definitions (`docs/superpowers/plans/2026-07-22-malformed-reference-definitions.md`) | Correctness follow-up; design `docs/superpowers/specs/2026-07-22-malformed-reference-definitions-design.md` | Three malformed reference-definition forms render as literal paragraphs matching the oracle: empty-label `[]:` and empty-URL `[not]:` over-strips in Act I; lossy Act III definition-replay for `[x] : destination`. Stacked on the ampersand PR branch | Four-input oracle parity on the real `./shakedown` binary; the three `test_act3_contracts` xfail(strict=True) cases pass without xfail; 23-fixture `test_mdtest` unchanged; literary + regen gates; full suite green | shipped: 2026-07-22 at commit 818cb52 |

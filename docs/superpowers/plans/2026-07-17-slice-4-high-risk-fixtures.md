...
The three §7.7 fixtures map one-to-one to Tasks 2–4; Task 1 proves the shipped floor and Task 5 supplies the full four-gate close. The accepted design supplies the bounded HTML surface, balanced quote/list grammar, A2 header prerequisite, A4's final-blank transaction and nested-after-blank selector, A5/A8's shared detabbed-depth accounting, and A9–A11's stage-pair-safe nested-to-outer sibling adapter plus a structural immediate-segment scan, with explicit no-new-token authority and a ready-to-paste literary pool whose 26-working/6-spare ledger is derived. Every SPL-facing task names generated-fragment, parse, validation, literary, Amps, fixture, strict-oracle, and spike gates; all fixture claims remain strict-byte claims.

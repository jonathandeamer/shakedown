...
- Carry list composition and nested block composition into the hardening probes; simple blockquote works in `./shakedown-dev`, but list looseness and nested composition are not SPL-proven.
...
- Keep emphasis backtracking as an explicit decision point, not an accidental property of the implementation; the prototype still marks the hard overlap case xfailed.
